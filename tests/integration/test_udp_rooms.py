...
@pytest.mark.parametrize(
    "room_id, door_state, should_exist",
    [
        ("LPA", DOOR_OPEN, True),
        ("LPA", DOOR_CLOSED, True),
        ("OFI", DOOR_OPEN, True),
        ("OFI", DOOR_CLOSED, True),
        ("NUL", DOOR_OPEN, False),
        ("NUL", DOOR_CLOSED, False),
    ],
)
def test_send_room_udp_telemetry(
    udp_client, room_id: str, door_state: int, should_exist: bool, visual_delay: bool
):
    ...
    try:
        ...
        # El paquete de respuesta mide al menos 2 bytes [0x5A, policy]
        assert len(data) >= 2, f"Respuesta UDP demasiado corta: {data.hex()}"
        
        # Magic byte de respuesta coincide con 0x5A
        assert data[0] == MAGIC_BYTE, f"Magic byte inválido: {hex(data[0])}"
        
        # Política energética válida (1=DEFAULT, 2=ENERGY_SAVER, 3=ULTRA_ENERGY_SAVER)
        policy = data[1]
